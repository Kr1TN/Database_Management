...
async function checkApiStatus() {
  try {
    ...
    document.getElementById('apiStatus').innerText = ok ? 'Connected to Supabase' : 'Connection Failed';
  } catch {
...
const DEPT_CLASS = {
  'Front Office':'dept-fo','Housekeeping':'dept-hk',
  'F&B':'dept-fb','Maintenance':'dept-mt','Security':'dept-sc'
};
...
async function loadEmployees() {
  ...
  try {
    ...
    renderEmployeeTable(Array.isArray(data)?data:[]);
  } catch(err) {
    console.error(err);
...
function renderEmployeeTable(data) {
  ...
  tbody.innerHTML = data.map(e=>`
    <tr>
      ...
      <td style="font-size:12px">${e.POSITION_NAME||'-'}</td>
      <td style="font-size:12px">${shiftIcon(e.SHIFT_TYPE)} ${e.SHIFT_TYPE||'-'}</td>
      <td style="color:var(--gold)">${fmt(e.MIN_SALARY)} – ${fmt(e.MAX_SALARY)} ฿</td>
      <td class="${e.STATUS==='Active'?'status-active':'status-inactive'}">${e.STATUS}</td>
      <td>
...
async function loadDropdowns() {
  try {
    ...
    // ✅ POSITION_ID เป็น 'P01','P02' ตรงๆ ใช้ value ตรงๆ ได้เลย
...
function openEmpModal() {
  ...
  ['empFirstName','empLastName','empPhone'].forEach(id=>{
...
async function editEmployee(id) {
  try {
    ...
    // ✅ POSITION_ID เป็น 'P01' อยู่แล้ว ใส่ตรงๆ ได้เลย ไม่ต้องแปลง
    document.getElementById('empPositionId').value = e.POSITION_ID || '';
    openModal('empModal');
  } catch(err) {
...
async function saveEmployee() {
  ...
  const body = {
    ...
    departmentId: deptId,
    // ✅ ส่ง 'P01','P02' ตรงๆ ไม่ต้อง .replace('P','') อีกต่อไป
    positionId:   posId,
  };
...
async function loadShiftDropdown() {
  try {
    ...
    // ✅ value เป็น 'S01','S02','S03' ตรงกับ DB
...
async function saveAttendance() {
  ...
  // ✅ S03 = กะดึก ตรวจจาก SHIFT_ID ตรงๆ
  ...
  try {
    const r = await fetch(`${API}/attendance`, {
      ...
      body: JSON.stringify({
        empId,
        // ✅ ส่ง 'S01' ตรงๆ ไม่ต้องแปลงเป็น '1' อีกต่อไป
        shiftId,
        workDate:  date,
        clockIn:   timeIn,
        clockOut:  timeOut,
